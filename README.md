...
| I want to… | My path |
|---|---|
| **Use AiNxt Code myself** — my own API key, my own IDE | ↓ Run the curl command below. Done in two minutes. |
| **Deploy AiNxt Code across a team** — governed access, shared budgets, audit, model policy | ↓ Install below, then [connect to the AiNxt gateway](#connect-a-model--then-open-the-panel) and set `--gateway` |
| **Build from source** — audit the code, contribute, or run an unreleased commit | [Install — full detail](#install--full-detail) → Route B — `curl ... \| sh -s -- --from-source` (Rust + Node toolchains, ~10–30 min) |
...
| I want to… | My path |
|---|---|
| **Fresh install** — no `ainxt` CLI yet | ↓ One command installs both the extension and the CLI agent |
| **Already have `ainxt` CLI** — just add the IDE extension | ↓ One command, skip the CLI install |
| **Pin a specific CLI version** | `curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh \| sh -s -- --cli-version 0.2.101` *(replace `0.2.101` with the version you want)* |
| **Verify before installing** | `curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh \| sh -s -- --verify` |

---

### Fresh install — extension + CLI agent in one command

No toolchain needed. Installs the editor extension **and** the `ainxt` CLI agent, then proves the chain works with a real ACP handshake.
...
> **Windows with Git Bash / MSYS2:** use the `curl | sh` form above.

When it finishes, open your IDE — the AiNxt panel appears in the Activity Bar (VS Code) or the right tool window (IntelliJ). Then [connect a model](#connect-a-model--then-open-the-panel).

---

### Already have `ainxt` CLI — just install the extension

**VS Code:**

```sh
curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh | sh -s -- --skip-cli
```

Or download [`ainxt-vscode.vsix`](https://github.com/npci/ainxt-code/releases/latest) and sideload it:
...
Download the plugin `.zip` from the [Releases](https://github.com/npci/ainxt-code/releases/latest) page, then install via **Settings → Plugins → ⚙ → Install Plugin from Disk**. See [Per-IDE detail](#per-ide-detail) for the full walkthrough.

---

### Pin a specific CLI version

Replace `0.2.101` with the version you want — the installer fetches that exact release:

```sh
curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh | sh -s -- --cli-version 0.2.101
```

### Verify before installing

Runs the installer's built-in checksum check — the binary is verified before it is placed on your machine:

```sh
curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh | sh -s -- --verify
```
...
### Option 1 — In-panel model picker: fastest, no file editing
...
### Option 2 — Environment variables: one-liner, nothing to edit
...
### Option 3 — `~/.ainxt/config.toml`: named models, permanent
...
| | |
|---|---|
| **Get started** | [Install](#install) · [① In-panel picker](#option-1--in-panel-model-picker-fastest-no-file-editing) · [② Env vars](#option-2--environment-variables-one-liner-nothing-to-edit) · [③ config.toml](#option-3--ainxtconfigtoml-named-models-permanent) · [Open the panel](#open-the-panel) |
...
| **Install detail** | [Route A vs B](#install--full-detail) · [The agent](#the-agent) · [Per-IDE detail](#per-ide-detail) |
...
**Not sure what you already have?** Run the installer with `--verify` — it checks each piece separately and tells you which one is missing, rather than failing with one unhelpful message:

```sh
curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh | sh -s -- --verify
```

There is a [Troubleshooting](#troubleshooting) section for when it does report a problem.
...
1. Install the `ainxt` CLI — run the one-liner from [`ainxt-cli`](https://github.com/npci/ainxt-cli),
   or let the IDE installer below handle it automatically.
...
3. Install the extension:
   ...
   **Or build from source** (after `git clone https://github.com/npci/ainxt-code.git`):
   ```sh
   cd vscode-acp
   npm ci
...
4. Open the **AiNxt** panel from the Activity Bar. When prompted to choose a profile,
   select **Standalone (no gateway)** if you are using the CLI directly — no gateway URL needed.
...
1. Install the `ainxt` CLI — run the one-liner from [`ainxt-cli`](https://github.com/npci/ainxt-cli), or let the IDE installer handle it automatically.
...
4. Open the **AiNxt** tool window (right dock). When prompted to choose a profile,
   select **Standalone (no gateway)** if you are using the CLI directly — no gateway URL needed.
...
Load one via the command palette: **AiNxt: Load Configuration Profile** (VS Code) or
**Settings → Tools → AiNxt → Load Configuration Profile…** (IntelliJ).

> **Just using the CLI?** Select **Standalone (no gateway)** — it is listed first and
> is the recommended default for individual developers. You do not need a gateway URL.
> Configure a model with `ainxt login`, `AINXT_API_KEY`, or `~/.ainxt/config.toml`,
> then start chatting. Choose the gateway profile only if your team runs the AiNxt
> Platform (`ainxt-enterprise`).

| Profile | When to choose it | Gateway | Model |
|---------|-------------------|---------|-------|
| `standalone.ainxtprofile.json` ✅ | **Using the CLI directly** — talking to a model you configure yourself (Anthropic, OpenAI, Ollama, …). Recommended for individual developers. | *(none)* | *(empty — configure in `~/.ainxt/config.toml` or the model picker)* |
| `oss.ainxtprofile.json` | Your team runs a self-hosted AiNxt Platform for shared auth, budgets, policy, and audit. | `http://localhost:8000` | *(empty — agent default)* |
...
guesses. Run `curl -fsSL https://raw.githubusercontent.com/npci/ainxt-code/main/install.sh | sh -s -- --verify` first — it reports the agent, sign-in and gateway
...
|---|---|
| `code: command not found` | VS Code's CLI is not on your `PATH` — the default on macOS. Command Palette → **"Shell Command: Install 'code' command in PATH"**, or use **Extensions panel → … → "Install from VSIX…"**. The installer finds the VS Code CLI itself and does not need this. |
| The panel opens but nothing happens when you send a prompt | Either no agent (this repository contains none — it spawns the `ainxt` binary; re-run the installer or point `ainxt.binaryPath` at an existing binary), or an agent with no model configured (`ainxt login`, `AINXT_API_KEY`, or a `[model.*]` entry in `~/.ainxt/config.toml` — no gateway required). |
